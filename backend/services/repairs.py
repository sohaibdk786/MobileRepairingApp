...
from backend.services.ticket_numbers import generate_next_ticket_number
...
def new_tracking_token() -> str:
    """Opaque token for the customer status QR -- not guessable from ticket #."""
    return secrets.token_urlsafe(16)


def ensure_tracking_token(conn: sqlite3.Connection, ticket: str) -> str:
    row = conn.execute(
        "SELECT tracking_token FROM repairs WHERE ticket = ?", (ticket,)
    ).fetchone()
    if row is None:
        raise ValueError(f"Ticket {ticket} not found")
    token = (row["tracking_token"] or "").strip()
    if token:
        return token
...
def get_repair_detail(conn: sqlite3.Connection, ticket: str) -> Optional[dict]:
    ...
    # Guarantee a token exists before any receipt print / track link.
    if not (detail.get("tracking_token") or "").strip():
        detail["tracking_token"] = ensure_tracking_token(conn, ticket)
    ...
    detail["track_url"] = build_track_url(shop, detail["tracking_token"])
    return detail
