...
def present_repair_detail(repair: dict, currency_code: str = "GBP") -> dict:
    ...
    detail["tracking_token"] = repair.get("tracking_token") or ""
    detail["track_url"] = repair.get("track_url") or ""
    return detail
...
def present_payment(payment: dict, currency_code: str = "GBP") -> dict:
    ...
    # payments table (backend.services.payments.add_refund) -- this flag is purely a
...
def present_sale(sale: dict, currency_code: str = "GBP") -> dict:
    ...
    return {
        ...
        "is_mobile": (sale["item"] or "").strip().lower() == "mobile phone",
    }


def present_phone(phone: dict, currency_code: str = "GBP") -> dict:
    """Staff shape for a phone listed for sale (includes IMEI)."""
    images = phone.get("images") or []
    return {
        "id": phone["id"],
        "created_at": phone["created_at"],
        "updated_at": phone["updated_at"],
        "model": phone["model"],
        "price_pence": phone["price_pence"],
        "price_display": format_pence(phone["price_pence"], currency_code),
        "notes": phone["notes"] or "",
        "imei": phone["imei"] or "",
        "listed": bool(phone["listed"]),
        "images": images,
