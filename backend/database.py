...
_MIGRATIONS: dict[str, list[tuple[str, str]]] = {
    ...
    "shop_settings": [
        ...
        # receipts alike) -- a code from backend.constants.CURRENCY_CHOICES
...
def get_connection() -> sqlite3.Connection:
    ...
    (dropfix_test.db or dropfix.db -- see backend.config).
...
def _migrate_currency_mode_to_code(conn: sqlite3.Connection) -> None:
    ...
    'USD' / 'PKR' -- see backend.constants.CURRENCY_CHOICES) once Tools >
...
def _migrate_merged_status(conn: sqlite3.Connection) -> None:
    ...
    backend.constants.STATUS_CHOICES). Any ticket already saved under one of
